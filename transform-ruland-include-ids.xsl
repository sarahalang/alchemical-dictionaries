        ...
        <entry>
            <xsl:attribute name="type">
                <xsl:choose>
                    <xsl:when test="descendant::t:form[@type = 'lemma']">
                        <xsl:value-of
                            select="substring(descendant::t:form[@type = 'lemma'][1], 1, 1)"/>
                    </xsl:when>
                    <!-- since //entry[ends-with(@n,'Ruland1612-')] yields nothing,
                    I'm assuming this covers all.. -->
                    <xsl:otherwise>
                        <xsl:value-of
                            select="substring(descendant::t:form[1], 1,1)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
            ...
            <xsl:attribute name="n">
                ...
                <xsl:choose>
                    <xsl:when test="descendant::t:form[@type = 'lemma']">
                        <xsl:value-of
                            select="concat('Ruland1612-', replace(descendant::t:form[@type = 'lemma'][1], ' ', '-'))"/>
                    </xsl:when>
                    <!-- since //entry[ends-with(@n,'Ruland1612-')] yields nothing,
                    I'm assuming this covers all.. 
                    ...
                    <xsl:otherwise>
                        <xsl:value-of
                            select="concat('Ruland1612-', replace(descendant::t:form[1], ' ', '-'))"/>
                    </xsl:otherwise>
                </xsl:choose>
